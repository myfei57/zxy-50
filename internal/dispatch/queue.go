...
func (q *CommandQueue) Replay() ([]Command, error) {
	q.mu.Lock()
	defer q.mu.Unlock()
	rows, err := q.store.ReadJournal(QueueKind)
	if err != nil {
	...
	replayed := make([]Command, 0, len(rows))
	for _, row := range rows {
		var command Command
		if err := json.Unmarshal(row, &command); err != nil {
			return nil, err
		}
		if err := q.store.AppendJSON(QueueKind, command); err != nil {
			return nil, err
		}
		replayed = append(replayed, command)
	}
	return replayed, nil
}
